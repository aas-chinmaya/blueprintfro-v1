...
// console.log("Rendering FundOverview with BudgetAccount:", BudgetAccount);

//   // Calculate values from BudgetAccount
//   const totalCredit = BudgetAccount.transactions
//     .filter(tx => tx.type === "credit" && !tx.isDeleted)
//     .reduce((sum, tx) => sum + tx.amount, 0);

//   const totalSpent = BudgetAccount.transactions
//     .filter(tx => tx.type === "debit" && !tx.isDeleted)
//     .reduce((sum, tx) => sum + tx.amount, 0);

//   const mainAccount = BudgetAccount.initialAmount + totalCredit;
//   const remaining = mainAccount - totalSpent;
//   const pendingFunds = totalCredit - totalSpent; // Funds allocated but not yet spent
//   const utilizationRatio = totalCredit > 0 ? (totalSpent / totalCredit) * 100 : 0;
...
//         </CardHeader>
//         <CardContent>
//           <div className="text-2xl">{formatCurrency(totalCredit)}</div>
//           <p className="text-xs text-muted-foreground mt-1">
