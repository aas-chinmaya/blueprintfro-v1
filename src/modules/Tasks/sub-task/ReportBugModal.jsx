...
const ReportBugModal = ({ onClose, task_id, subtask_id, isOpen = true }) => {
  ...
  const handleReportBug = async () => {
    ...
    // Prepare form data for optional file
    ...
    try {
      ...
      const errorMessage = err?.message || "Failed to create bug. Please try again.";
      toast.error(errorMessage);
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="w-full max-w-full h-[100vh] sm:max-w-6xl sm:max-h-[85vh] bg-white shadow-lg border border-gray-200 rounded-lg p-2">
        <DialogHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-3 border-b sticky top-0 z-10 flex items-center justify-between">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleReportBug(); }} className="space-y-4">
            ...
            <div>
              <label className="flex items-center text-sm font-medium mb-2">
                <Bug className="h-4 w-4 text-blue-500 mr-2" /> Title <span className="text-red-500 ml-1">*</span>
              </label>
              <Textarea
                value={bugTitle}
                onChange={(e) => { setBugTitle(e.target.value); setErrors(prev => ({ ...prev, title: '' })); }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                ...
                <Select value={priority} onValueChange={(value) => setPriority(value)} disabled={isSubmitting}>
              ...
              <div>
                <label className="flex items-center text-sm font-medium mb-2">
                  <CalendarIcon className="h-4 w-4 text-blue-500 mr-2" /> Date <span className="text-red-500 ml-1">*</span>
                </label>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="w-full h-10">{selectedDate ? format(selectedDate, "MMM dd, yyyy") : "Select date"}</Button>
                  </PopoverTrigger>
                  <PopoverContent>
              ...
              <div>
                ...
                <Input type="time" value={selectedTime} onChange={(e) => setSelectedTime(e.target.value)} className="w-full h-10" disabled={isSubmitting} />
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea value={bugDescription} onChange={(e) => setBugDescription(e.target.value)} className="w-full h-[40vh] p-3" placeholder="Describe the issue..." disabled={isSubmitting} />
            </div>

            {/* File Attachment */}
            <div>
              <label className="flex items-center text-sm font-medium mb-2">Attachment (optional)</label>
              <Input type="file" onChange={(e) => setAttachment(e.target.files[0])} disabled={isSubmitting} />
            </div>
